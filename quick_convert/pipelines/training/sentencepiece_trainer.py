...
class TokenizerTrainer(BaseTrainer):
    ...
    def _iter_sentences(self, dataset: Any) -> Iterable[str]:
        for item in dataset:
            if hasattr(item, self.text_key):
                text = getattr(item, self.text_key)
    ...
    def train(
        ...
        **kwargs: Any,
        ...
        if train_dataset is None:
            raise ValueError("TokenizerTrainingSystem requires `train_dataset`.")

        resolved_output_dir = self.resolve_output_dir(output_dir)
        if resolved_output_dir is None:
            raise ValueError("No output_dir was provided.")

        return self.tokenizer_trainer.train_from_iterator(
