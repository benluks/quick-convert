...
class UpsampleConformerEncoder(torch.nn.Module):
    def __init__(
        ...
        self.up_layer = Upsample1D(channels=512, out_channels=512, stride=2) if do_upsample else None
        self.up_embed = COSYVOICE_SUBSAMPLE_CLASSES[input_layer](
            # this was originally `input_size` which must have been an error
            output_size,
            output_size,
            dropout_rate,
            COSYVOICE_EMB_CLASSES[pos_enc_layer_type](output_size, positional_dropout_rate),
    ...
    def forward(
        ...
        mask_pad = masks  # (B, 1, T/subsample_rate)
        chunk_masks = add_optional_chunk_mask(
            xs, masks, False, False, 0, self.static_chunk_size * self.upsample_stride if streaming is True else 0, -1
        )
