...
class MaskedDiffWithXvec(torch.nn.Module):
    ...
    def forward(
        ...
        loss, _ = self.decoder.compute_loss(
            feat.transpose(1, 2).contiguous(), mask.unsqueeze(1), h.transpose(1, 2).contiguous(), embedding, cond=conds
        )
        return {"loss": loss}
...
class CausalMaskedDiffWithXvec(torch.nn.Module):
    def __init__(
        ...
        self.input_frame_rate = input_frame_rate
        logging.info(f"input frame rate={self.input_frame_rate}")
        self.input_embedding = nn.Embedding(vocab_size, input_size) if vocab_size is not None else None
        self.spk_embed_affine_layer = torch.nn.Linear(spk_embed_dim, output_size)
        self.encoder = encoder
        self.encoder_proj = torch.nn.Linear(self.encoder.output_size(), output_size)
        self.decoder = decoder
        ...
        if online_feature is True:
            self.speech_token_extractor = SpeechTokenExtractor(
                model_path=os.path.join(onnx_path, "speech_tokenizer_v2.batch.onnx")
            )
    ...
    def inference(
        ...
        if finalize is True:
            ...
            encoder_token_len = torch.clamp(token_len - self.pre_lookahead_len, min=0)
            h, h_lengths = self.encoder(token, encoder_token_len, context=context, streaming=streaming)
        ...
        assert prompt_feat.shape[0] == b
        assert prompt_token.shape[0] == b
        assert embedding.shape[0] == b

        assert torch.all(prompt_feat_len == prompt_feat_len[0]), (
            "Batched inference currently requires equal-length prompt features"
        )

        assert torch.all(prompt_token_len == prompt_token_len[0]), (
            "Batched inference currently requires equal-length prompt tokens"
        )
        assert prompt_feat.shape[1] == int(prompt_feat_len[0]), (
            "Batched inference currently expects prompt_feat to contain no padding"
        )
        assert prompt_token.shape[1] == int(prompt_token_len[0]), (
            "Batched inference currently expects prompt_token to contain no padding"
        )

        conds = torch.zeros(
            [b, mel_len1 + mel_len2, self.output_size],
            device=token.device,
            dtype=h.dtype,
        )
        conds[:, :mel_len1] = prompt_feat
        ...
        mel_lens = torch.full((b,), mel_len1 + mel_len2, device=h.device, dtype=torch.long)
...
class CausalMaskedDiffWithDiT(torch.nn.Module):
    ...
    def forward(
        ...
        # text encode
        h = self.pre_lookahead_layer(token)  # B, T, D
...
if __name__ == "__main__":
    ...
    for i in range(0, max_len, chunk_size):
        ...
        pred_chunk, _ = model.inference(
            token[:, : i + chunk_size + context_size],
            torch.tensor([token[:, : i + chunk_size + context_size].shape[1]]).to(device),
            prompt_token,
            prompt_token_len,
            prompt_feat,
            prompt_feat_len,
        ...
        print(
            (pred_gt[:, :, i * model.token_mel_ratio : i * model.token_mel_ratio + pred_chunk.shape[2]] - pred_chunk)
            .abs()
            .max()
            .item()
        )
