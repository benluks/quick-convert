...
class ECAPA_TDNN(torch.nn.Module):
    ...
    def __init__(
        ...
        self.blocks_classifier = nn.ModuleList()

        for block_index in range(lin_blocks):
            self.blocks_classifier.extend(
                [
                    _BatchNorm1d(input_size=input_size_classifier),
                    Linear(input_size=input_size_classifier, n_neurons=lin_neurons),
                ]
            )
            input_size_classifier = lin_neurons

        # Final Layer
        # self.weight = nn.Parameter(
        #    torch.FloatTensor(out_neurons, input_size_classifier, device=device)
        # )
    ...
    def forward(self, x, lengths=None):
        ...
        x = x.transpose(1, 2)
        features = x
        for layer in self.blocks_classifier:
            x = layer(x)

        # Need to be normalized
        # x = F.linear(F.normalize(x.squeeze(1)), F.normalize(self.weight))
        return features, x.unsqueeze(1)
