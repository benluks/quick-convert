"""
TAKEN FROM https://github.com/xiaoxiaomiao323/emotion-compensation/blob/main/gen/adapted_from_facebookresearch/models_test.py
"""
# ruff: noqa: E741
...
def state_dict_wrapper_for_ecapa(stat_dict, new_model):
    ...
    if mismatch_keys:
        print("Partially load model, ignoring buffers: {:s}".format(" ".join(mismatch_keys)))
...
class latentGenerator(Generator):
    def __init__(self, h):
        ...
        if h.get("soft_model_path", None) and h.get("hubert_model_path", None):
            ...
            self.feat_model.remove_pretraining_modules()
            set_parameter_requires_grad(
                self.feat_model, h.get("ssl_freeze", None)
            )  # ssl_freeze=True not finetune ssl model
