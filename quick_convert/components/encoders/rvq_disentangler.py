...
from quick_convert.external.cosyvoice.transformer.upsample_encoder import UpsampleConformerEncoder
...
class RVQDisentanglerLoss:
    ...
    # formerly mse_loss; measures how well the quantized representation reconstructs the content representation
    supervised: dict[str, torch.Tensor]
    latent_reconstruction: torch.Tensor | None = None
...
class RVQDisentangler(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.latent_reconstruction_loss_weight = latent_reconstruction_loss_weight
    ...
    def encode(
        self,
        features: int["b t [l] d"],
        padding_mask: int["b t [1]"],
        lengths: int["b"] | None = None,
    ) -> RVQDisentanglerOutput:

        # layer reduction of layered ssl features
        if self.layer_transform is not None:
            features = self.layer_transform(features)
        # UpsampleConformerEncoder takes lens rather than maks as an argument
        content, masks = self.content_encoder(
            features,
            padding_mask.sum(1),
            streaming=False,
        )

        # Depending on the exact shape/semantics returned by your
        # UpsampleConformerEncoder, normalize this here.
        lengths = masks.sum(dim=-1).squeeze(1)

        max_length = int(lengths.max().item())
        content = content[:, :max_length]
        padding_mask = masks.transpose(1, 2)[:, :max_length]
    ...
    def compute_loss(
        ...
        # relic from old design. There is a similar type of mse loss in the new rvq_ema implementation
        ...
        loss = RVQDisentanglerLoss(
            rvq=rvq_losses,
            latent_reconstruction=reconstruction_loss * self.latent_reconstruction_loss_weight,
            load_balancing=output.router.loss,
