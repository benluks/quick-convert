...
from torch import Tensor, distributed, einsum, is_tensor, nn, tensor
...
def ema_inplace(old, new, decay, weight=None):
    ...
    old.data.lerp_(new, (1.0 - decay) * weight)
...
def gumbel_sample(logits, temperature=1.0, stochastic=False, straight_through=False, dim=-1, training=True, topk=None):
...
def rotate_to(src, tgt):
    ...
    rotated_tgt = efficient_rotation_trick_transform(safe_div(src, norm_src), safe_div(tgt, norm_tgt), src)
...
class Codebook(Module):
    ...
    def track_cluster_size_and_embed_avg(
        ...
        if exists(mask):
            embed_onehot = embed_onehot.masked_fill(~rearrange(mask, "... -> ... 1"), 0.0)
    ...
    def forward(
        ...
        embed_ind, embed_onehot = self.gumbel_sample(
            dist, dim=-1, topk=topk, temperature=sample_codebook_temp, training=self.training
        )
...
class VectorQuantize(Module):
    def __init__(
        ...
        rotation_trick = default(
            rotation_trick, not directional_reparam and dim > 1
        )  # only use rotation trick if feature dimension greater than 1
        ...
        self.has_commitment_loss = commitment_weight > 0.0 and not directional_reparam
        ...
        assert not (directional_reparam and threshold_ema_dead_code == 0), (
            "periodic dead code replacement should be enabled when differential reparam method is turned on"
        )
        ...
        assert not (straight_through and learnable_codebook), (
            "gumbel straight through not allowed when learning the codebook"
        )
        assert not (ema_update and learnable_codebook), "learnable codebook not compatible with EMA update"
        assert not (exists(vq_bridge) and not learnable_codebook), (
            "learnable_codebook must be set to True if vq_bridge is passed in"
        )
    ...
    def forward(
        ...
        loss = tensor(0.0, device=device, requires_grad=self.training)
            ...
            if self.has_commitment_loss:
                if self.commitment_use_cross_entropy_loss:
                    ...
                    if exists(topk):
                        ...
                        if exists(mask):
                            commit_loss = einx.where("..., ... k, -> ... k", mask, commit_loss, 0.0)
