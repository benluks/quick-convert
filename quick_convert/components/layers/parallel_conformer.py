...
class ParallelConformerEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.conformer_encoders = [ConformerBlock(embed_dim=embed_dim,
                                            num_heads=num_sub_heads, 
                                            ffn_dim=ffn_dim, 
                                            conv_kernel_size=conv_kernel_size, 
                                            dropout=dropout, 
                                            bias=bias,
                                            pos_emb_base=pos_emb_base) for _ in range(num_meta_heads)]
    ...
    def forward(
        ...
        x = torch.cat([block(x[:, :, i, :], padding_mask) for i, block in enumerate(self.conformer_blocks)], dim=-1)
